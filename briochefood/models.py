...
class User(db.Model, SerializerMixin):    
    ...
    status = db.Column(db.String(20), db.Enum('ACTIVE', 'BLOCKED'), default='ACTIVE', nullable=False)
...
class User_Address(db.Model, SerializerMixin):
    ...
    user = db.relationship(User, backref=db.backref("users_addresses", cascade="all, delete-orphan"))
    address = db.relationship(Address, backref=db.backref("users_addresses", cascade="all, delete-orphan"))
...
class Bakery(db.Model, SerializerMixin):     
    ...
    email = db.Column(db.String(128), unique=True, nullable=False)
    password = db.Column(db.String(128), nullable=False)            
    status = db.Column(db.String(20), db.Enum('ACTIVE', 'INACTIVE', 'BLOCKED'), default='ACTIVE', nullable=False)
    address_id = db.Column(db.Integer, db.ForeignKey('addresses.id'), nullable=False)     
    created_at = db.Column(db.DateTime, default=datetime.datetime.now, nullable=False)
    updated_at = db.Column(db.DateTime, default=None)
...
class Product(db.Model, SerializerMixin):
    ...
    status = db.Column(db.String(20), db.Enum('ACTIVE', 'INACTIVE'), default='ACTIVE', nullable=False)
...
class Purchase(db.Model, SerializerMixin):      
    ...
    status = db.Column(db.String(20), db.Enum('PAID', 'CANCELED', 'REVERSAL'), default='PAID', nullable=False)        
    cart_id = db.Column(db.Integer, db.ForeignKey('carts.id'), nullable=False) 
    created_at = db.Column(db.DateTime, default=datetime.datetime.now, nullable=False)
    updated_at = db.Column(db.DateTime, default=None)
...
class Delivery(db.Model, SerializerMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)    
    status = db.Column(db.String(20), db.Enum('PENDENT', 'CANCELED', 'INPROGRESS', 'DELIVERED'), default='PENDENT', nullable=False)
    note = db.Column(db.String(200))
